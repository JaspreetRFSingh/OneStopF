...
public class EventPlannerActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mContext = this;
        progressBar = findViewById(R.id.progressBarEventPlanner);
        progressBar.setVisibility(View.VISIBLE);
        android.support.v7.widget.Toolbar myToolbar = findViewById(R.id.my_toolbar);
        myToolbar.setTitle("EventPlanners");
        ...
        rcvShowEvPlannerType = getIntent().getExtras().getString("keyShowEventType");
        dbRef = FirebaseDatabase.getInstance().getReference().child("EventPlanners").child(rcvShowEvPlannerType);
        ...
        if (ActivityCompat.checkSelfPermission(EventPlannerActivity.this, android.Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED && ActivityCompat.checkSelfPermission(EventPlannerActivity.this, android.Manifest.permission.ACCESS_COARSE_LOCATION) != PackageManager.PERMISSION_GRANTED) {
            Toast.makeText(EventPlannerActivity.this, "Please grant location permissions in settings", Toast.LENGTH_LONG).show();
        } else {
        ...
        adapterEvPlanner = new FirebaseRecyclerAdapter<EventPlanner, ViewHolderEventPlanner>(options) {
            ...
            protected void onBindViewHolder(ViewHolderEventPlanner viewHolder, int position, final EventPlanner model) {
                ...
                viewHolder.bindToEventPlanner(model, mContext, myDistance.getDistance());
            }
        };
        recyclerViewEvPlanner.setAdapter(adapterEvPlanner);
        recyclerViewEvPlanner.startAnimation(AnimationUtils.loadAnimation(getApplicationContext(), R.anim.bounce));}
    @Override
    public void onStart() {
        super.onStart();
